    ...
    void Dv_nozzle::initialization (double P_0, double T_0)
{
        ...
        for (int i=1;i<im_c;i++)
        {
            ...
            r_minus= (_V[0][i+1]-_V[0][i])/(_V[0][i]-_V[0][i-1]);
            ...
            r_minus= (_V[1][i+1]-_V[1][i])/(_V[1][i]-_V[1][i-1]);
            ...
            r_minus= (_V[2][i+1]-_V[2][i])/(_V[2][i]-_V[2][i-1]);
        ...
        for (int i=2;i<=im_c-1;i++)
        {
            ...
            //density
            V_L[0]= _V[0][i-1]+(epsilon_upwind/4)*((1-kappa)*epsi_plus[0][i]*(_V[0][i-1]-_V[0][i-2]) + (1+kappa)*epsi_minus[0][i+1]*(_V[0][i]-_V[0][i-1])        );
            
            V_R[0]= _V[0][i]+(epsilon_upwind/4)*((1-kappa)*epsi_plus[0][i+1]*(_V[0][i+1]-_V[0][i]) + (1+kappa)*epsi_plus[0][i]*(_V[0][i]-_V[0][i-1])        );
            //velocity
            V_L[1]= _V[1][i-1]+(epsilon_upwind/4)*((1-kappa)*epsi_plus[1][i]*(_V[1][i-1]-_V[1][i-2]) + (1+kappa)*epsi_minus[1][i+1]*(_V[1][i]-_V[1][i-1])        );
            
            V_R[1]= _V[1][i]+(epsilon_upwind/4)*((1-kappa)*epsi_plus[1][i+1]*(_V[1][i+1]-_V[1][i]) + (1+kappa)*epsi_plus[1][i]*(_V[1][i]-_V[1][i-1])        );
            //pressure
            V_L[2]= _V[2][i-1]+(epsilon_upwind/4)*((1-kappa)*epsi_plus[2][i]*(_V[2][i-1]-_V[2][i-2]) + (1+kappa)*epsi_minus[0][i+1]*(_V[0][i]-_V[0][i-1])        );
            
            V_R[2]= _V[2][i]+(epsilon_upwind/4)*((1-kappa)*epsi_plus[2][i+1]*(_V[2][i+1]-_V[2][i]) + (1+kappa)*epsi_plus[2][i]*(_V[2][i]-_V[2][i-1])        );
            //Temperature
            V_L[2]= _V[2][i-1]+(epsilon_upwind/4)*((1-kappa)*epsi_plus[2][i]*(_V[2][i-1]-_V[2][i-2]) + (1+kappa)*epsi_minus[0][i+1]*(_V[0][i]-_V[0][i-1])        );
...
void Dv_nozzle::set_boundary_cond()
{
   ...
   // et_in = (R_air/(gamma-1))*T_in + 0.5*u_in*u_in;
    ...
    _V_ghost_inflow[0]=2*_V[0][_imin]-_V[0][im_c-1];
    _V_ghost_inflow[1]=2*_V[1][_imin]-_V[1][im_c-1];
    
    _V_ghost_inflow[2]=2*_V[2][_imin]-_V[2][_imin+1];
    _V_ghost_inflow[3]=2*et[_imin]-et[_imin+1];  //et
    _V_ghost_inflow[4]=2*T[_imin]-T[_imin+1];    //T
...
//******************************************
  ...
  // std::cout<<"back pressure is "<<P_out<<std::endl;
   ...
   _F[2][im_f]= (gamma/(gamma-1))*u_out*P_out + (rho_out*u_out*u_out*u_out)/2;
    ...
    _F[0][im_f-1] = 0.5*(_F[0][im_f] + _F[0][im_f-2]);
    _F[1][im_f-1]=  0.5*(_F[1][im_f] + _F[1][im_f-2]);
    _F[2][im_f-1]=  0.5*(_F[2][im_f] + _F[2][im_f-2]);
  ...
  /*
    //extrapolate U and V for 1st ghost cell
    _U_ghost_outflow[0]=2*_U[0][im_c]-_U[0][im_c-1];
    _U_ghost_outflow[1]=2*_U[1][im_c]-_U[1][im_c-1];
    _U_ghost_outflow[2]=2*_U[2][im_c]-_U[2][im_c-1];
    
    _V_ghost_outflow[0]=2*_V[0][im_c]-_V[0][im_c-1];
    _V_ghost_outflow[1]=2*_V[1][im_c]-_V[1][im_c-1];
    _V_ghost_outflow[2]=2*P_out-_V[2][im_c];
    _V_ghost_outflow[3]=2*et[im_c]-et[im_c-1];  //et
   ...
   // P_out = 0.5*(_V_ghost_outflow[2]+_V[2][im_c]);
  // std::cout<<"back pressure is "<<P_out<<std::endl;
  /*
    P_out = 125000;
    rho_out = 0.5*(_U_ghost_outflow[0]+_U[0][im_c]);
    
    
   
    u_out = (0.5*(_U_ghost_outflow[1]+_U[1][im_c]))/rho_out;
...
void Dv_nozzle::euler_explicit()
{
    ...
    _U[0][i]= ( (-_F[0][i+1]   ) *area(x_f[i+1]) + (   _F[0][i] + _D[0][i]   )*area(x_f[i])    )*
               
               
              ( delta_t[i]/(avg_area *_delta_x)) + _U[0][i];
           
        
    _U[1][i]= (  (-_F[1][i+1]    )*area(x_f[i+1]) + (   _F[1][i] + _D[1][i]   )*area(x_f[i]) +_V[2][i]*avg_d_area*_delta_x    )*(delta_t[i]/(avg_area*_delta_x)) + _U[1][i];
        
        
    _U[2][i]= (( -_F[2][i+1] ) *area(x_f[i+1]) + (_F[2][i] + _D[2][i]  )*area(x_f[i]))*(delta_t[i]/(avg_area *_delta_x)) + _U[2][i];
